...
In this case, median is the best single number to compare income because it is less sensitive to outliers (though looking at all of the numbers is important to see how direction outliers skew the data)
...
return(list(treated.shift, nottreated.shift, sate))
}
sateA(boston)
...
sate.coll <- sateB(college)
sate.nocoll <- sateB(no.college)

sate.coll - sate.nocoll
...
```{r warning=FALSE}
spend.clean <- subset(prop.AJPS, spending >= 0) #cleaning up the spending data to remove those pesky negative values.
