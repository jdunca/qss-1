...
stargazer::stargazer(mun_fit, mun_fit2, mun_fit3, type = "html", title = "Model Outputs")
...
```{r message=FALSE, warning=FALSE}
costs %>% 
  select (Y, year, treatment) %>% 
  group_by(year, treatment) %>% 
  summarise(mean.outcome = mean(Y)) %>% 
  mutate(treatment = recode(treatment, "0" = "Not treated", "1" = "Treated")) %>% 
  ggplot(aes(x = year, y = mean.outcome, group = treatment))+
    geom_point(aes(shape = factor(treatment), colour = factor(treatment)))+
    geom_line(aes(colour = factor(treatment)))+
    geom_vline(xintercept= 2007, linetype="dashed", color = "grey")+
    theme_minimal()+
    theme(legend.title = element_blank(), legend.position = "top")+
    labs(title = "Mean outcome by year", 
