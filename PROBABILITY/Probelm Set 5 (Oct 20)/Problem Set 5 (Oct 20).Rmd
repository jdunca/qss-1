...
realobama.ev <- rep(NA, sims)
for (i in 1:sims){
  draws <- rbinom(n.states, size = n, prob = pred08.rel$probama)
  realobama.ev[i] <- sum(pred08.rel$EV[draws > n/ 2])
}

predobama.ev <- rep(NA, sims)
for (i in 1:sims){
  draws <- rbinom(n.states, size = n, prob = pred08.rel$ppd)
  predobama.ev[i] <- sum(pred08.rel$EV[draws > n/ 2])
}
hist(realobama.ev, freq = F, main = "Prediction of election outcome (real)", xlab = "Obama's Electoral College votes")
...
for (i in 1:sims) {
  draws <- rbinom(sims, size = 1, prob = pred08.rel$ppd)
  pred08.rel$win[i, ] <- mean(draws)
}
...
###### Question 3
Consider two data generating processes. In one, people have some true trust level that is uniformly distributed as one of 11 levels, in order. That is, the scale, running from 0 to 10, reflects the trust level that people actually have, and people are spread evenly across those groups. Alternatively, consider a model in which Y   N(0, 1) and yi is 8>< >: 0 if y i < 0 j if j−1 < y i  j 10 if y i > 8

That is,  is a vector of length 9 (numbered 0 to 8), and Y  is an unobserved (latent) variable that is “revealed” to the observer through the survey as Y depending on where it falls among the taus. To visualize this, consider the Figure 1. The density of Y  is overlaid with the “buckets” into which y is grouped. Simulate 1000 observations from each of these DGPs. Use  = seq(−2, 2.5, by = .5) for the latent variable. make barplots and compare them to the one of ownchtrust. Then compare each of the new distributions to ownchtrust using a qqplot.
